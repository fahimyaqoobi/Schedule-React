...
//
// `attachment`, when given, is { filename, mimeType, data: Buffer } — the
// reply becomes multipart/mixed (a text part + the file, base64-encoded).
// Whether Google's Local Services relay actually forwards a file through to
// the real customer (vs. just the text) isn't documented anywhere — worth
// confirming with a real test send.
export function buildReplyRawMessage({ to, subject, bodyText, inReplyTo, references, attachment }) {
    const subjectLine = subject && subject.startsWith("Re:") ? subject : `Re: ${subject || "your request"}`;
    const baseHeaders = [
        `To: ${to}`,
        `Subject: ${subjectLine}`,
    ...
    const boundary = `stc-boundary-${Date.now().toString(36)}`;
    ...
    // Wrapped at 76 chars per RFC 2045 — some mail relays reject unwrapped
    // base64 lines.
    const encodedAttachment = attachment.data.toString("base64").replace(/(.{76})/g, "$1\r\n");
