...
export async function buildPaystubPdf({ employee = {}, period = {}, rows = [], breakdown = {}, company = {}, payrollRecord = null }, options = {}) {
    ...
    addText(`${formatDateLong(period.periodStart)} – ${formatDateLong(period.cutoffDate)}`, rightCardX, cardTop + 14, {
    ...
    if (payrollRecord?.paidAt) {
        addText(`Paid on ${formatDateLong(new Date(payrollRecord.paidAt))}`, rightCardX, cardTop + 58, { size: SIZE.small, color: SLATE });
    }
    ...
    rows.forEach((row) => {
        const dateText = formatDateShort(row.date);
        ...
        cursorX += colWidths[1];
        addText(formatClockTime(row.timeIn), cursorX + cellPad, y + 6, { size: rowFont, width: colWidths[2] - (cellPad * 2) });
        cursorX += colWidths[2];
        addText(formatClockTime(row.timeOut), cursorX + cellPad, y + 6, { size: rowFont, width: colWidths[3] - (cellPad * 2) });
        cursorX += colWidths[3];
