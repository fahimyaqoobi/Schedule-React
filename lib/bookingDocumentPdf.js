...
export async function buildBookingDocumentPdf(booking = {}, options = {}) {
    ...
    // ── SCHEDULED SERVICE banner: date, arrival time, and frequency must be
    //    unmissable for both the client and the crew.
    y = cardTop + 98;
    const SHIFT_HOURS = {
    ...
    const bannerHeight = shiftText ? 66 : 56;
    doc.roundedRect(margin, y, contentWidth, bannerHeight, 8).fillColor(pale).fill();
    doc.roundedRect(margin, y, contentWidth, bannerHeight, 8).lineWidth(1.5).strokeColor(green).stroke();
    const bannerColWidth = contentWidth / 3;
    addText("SERVICE DATE", margin + 16, y + 10, { size: 9, color: slate, font: "Helvetica-Bold" });
    addText(formatDate(booking.date) || "To be scheduled", margin + 16, y + 24, {
        size: 15,
        color: navy,
        font: "Helvetica-Bold",
        width: bannerColWidth - 24
    });
    addText("ARRIVAL TIME", margin + 16 + bannerColWidth, y + 10, { size: 9, color: slate, font: "Helvetica-Bold" });
    addText(booking.time || "To be confirmed", margin + 16 + bannerColWidth, y + 24, {
        size: 15,
        color: navy,
        font: "Helvetica-Bold",
        width: bannerColWidth - 24
    });
    addText("FREQUENCY", margin + 16 + (bannerColWidth * 2), y + 10, { size: 9, color: slate, font: "Helvetica-Bold" });
    addText(
        booking.isRecurring ? `${booking.frequency || "Recurring"} (Recurring)` : (booking.frequency || "One-Time"),
        margin + 16 + (bannerColWidth * 2), y + 24, {
            size: 15,
            color: navy,
            font: "Helvetica-Bold",
            width: bannerColWidth - 24
    ...
    if (shiftText) {
        addText(shiftText, margin + 16, y + 46, { size: 9, color: slate, width: contentWidth - 32 });
    }
    y += bannerHeight + 16;
    ...
    addText("TOTAL", totalsX + 16, totalBoxY + 14, {
        size: 18,
        color: "#ffffff",
        font: "Helvetica-Bold",
        width: 160
