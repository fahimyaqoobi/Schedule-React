...
export async function buildBookingDocumentPdf(booking = {}, options = {}) {
    ...
    totalLines.forEach(([label, value], index) => {
        const lineY = y + 2 + (index * 18);
        addText(label, totalsX, lineY, { size: 11, width: 150 });
        addText(value, totalsX + 150, lineY, { size: 11, font: "Helvetica-Bold", width: 130, align: "right" });
    });
    const totalBoxY = y + 12 + (totalLines.length * 18);
    ...
    addText(formatMoney(context.pricing.total), totalsX + 160, totalBoxY + 14, {
        size: 22,
        color: "#ffffff",
        font: "Helvetica-Bold",
        width: 104,
    ...
    y += Math.max(118, 32 + (totalLines.length * 18) + 44);
    const columnGap = 30;
