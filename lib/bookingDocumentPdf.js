...
export async function buildBookingDocumentPdf(booking = {}, options = {}) {
    ...
    const detailRows = [
        ...
        ["Valid Until:", context.validUntil ? `${context.validUntil} (30 Days)` : "30 Days from issue", "#1a2942"]
    ];
    ...
    context.pricing.services.forEach((item) => {
        const rowHeight = 40 + (item.addonLines.length * 13);
        ...
        cells.forEach((cell, cellIndex) => {
            addText(cell, cursorX + 8, y + 8, {
                size: SIZE.body,
                color: cellIndex === 3 ? navy : "#1a2942",
        ...
        addText("Base service price", tableX + colWidths[0] + 8, y + 21, {
            size: SIZE.small - 0.5,
            color: slate,
            width: colWidths[1] + colWidths[2] - 16
        });
        addText(formatMoney(item.baseAmount), tableX + colWidths[0] + colWidths[1] + colWidths[2] + 8, y + 21, {
            size: SIZE.small - 0.5,
            color: navy,
            font: "Helvetica-Bold",
        ...
        item.addonLines.forEach((addon, addonIndex) => {
            const addonY = y + 32 + (addonIndex * 13);
