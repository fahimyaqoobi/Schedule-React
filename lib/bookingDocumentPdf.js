...
export async function buildBookingDocumentPdf(booking = {}, options = {}) {
    ...
    try {
        if (options.logoBuffer) {
            doc.image(options.logoBuffer, margin, y, { fit: [180, 70] });
        }
    } catch (_error) {
    ...
    addText(context.company.branchName || booking.branchName || "Ottawa", margin, y + 86, { size: 12 });
    addText(context.branchEmail, margin, y + 106, { size: 12 });
    ...
    addText("Refer a friend and you both get $30 OFF your next cleaning service.", margin + 54, y + 34, {
        size: 10.5,
        color: "#2f3f58",
        width: promoWidth - 68,
        lineGap: 2
    ...
    doc.moveTo(noteX, termsY + 78).lineTo(noteX + columnWidth, termsY + 78).lineWidth(1).strokeColor("#5c6670").stroke();
    addText("Client Signature                                Date", noteX, termsY + 84, { size: 9.5, color: slate });
