...
// Matches the "valid for N days" clause before {document} substitution —
// only an Estimate is a time-limited offer that can expire; an Invoice,
// Booking confirmation, or Receipt isn't an "offer" at all, so showing an
// expiry date on those just confuses the customer ("wait, is my booking
// about to expire?"). Matched by prefix (not the whole line) so it still
// catches this clause if an admin edits just the day count in Settings.
const VALID_FOR_DAYS_PATTERN = /^This \{document\} is valid for/i;
...
export function getDocumentTerms(copy = {}, documentLabel = "document") {
    ...
    return normalized.termsBody
        ...
        .map((line) => line.trim())
        .filter(Boolean)
        .filter((line) => documentLabel === "Estimate" || !VALID_FOR_DAYS_PATTERN.test(line))
