const STAFF_PROFILE_TEMPLATE = {
    ...
    availability: {
        maxJobsPerDay: 3,
        weekdays: [
            { label: "M", enabled: true, shifts: { morning: true, afternoon: true, evening: false } },
            { label: "T", enabled: true, shifts: { morning: true, afternoon: true, evening: false } },
            { label: "W", enabled: true, shifts: { morning: true, afternoon: true, evening: false } },
            { label: "T", enabled: true, shifts: { morning: true, afternoon: true, evening: false } },
            { label: "F", enabled: true, shifts: { morning: true, afternoon: true, evening: false } },
            { label: "S", enabled: false, shifts: { morning: false, afternoon: false, evening: false } },
            { label: "S", enabled: false, shifts: { morning: false, afternoon: false, evening: false } }
        ],
        blockedDates: []
    },
...
const DAY_SHIFT_TEMPLATE = { morning: true, afternoon: true, evening: false };
const OFF_DAY_SHIFT_TEMPLATE = { morning: false, afternoon: false, evening: false };

export function getAvailabilityWeekdayIndex(dateWeekday) {
...
export function normalizeStaffProfile(profile = {}) {
    ...
    const legacyShifts = availability.shifts || {};
    return {
        ...
        availability: {
            ...
            weekdays: Array.isArray(availability.weekdays) && availability.weekdays.length
                ? availability.weekdays.map((day, index) => ({
                    ...
                    ...day,
                    shifts: {
                        ...(STAFF_PROFILE_TEMPLATE.availability.weekdays[index]?.enabled === false ? OFF_DAY_SHIFT_TEMPLATE : DAY_SHIFT_TEMPLATE),
                        ...(legacyShifts || {}),
                        ...(day?.shifts || {})
                    }
                }))
                : STAFF_PROFILE_TEMPLATE.availability.weekdays.map(day => ({
                    ...day,
                    shifts: { ...(day.enabled ? DAY_SHIFT_TEMPLATE : OFF_DAY_SHIFT_TEMPLATE) }
                })),
...
function hasAvailabilityWindowConflictForDate(currentAvailability, nextAvailability, date) {
    ...
    if (JSON.stringify(currentDay?.shifts || {}) !== JSON.stringify(nextDay?.shifts || {})) {
        return true;
    }
...
export function buildAvailabilitySnapshot(availability = {}) {
    ...
    const weekdayStatus = normalized.weekdays.map((day, index) => ({
        ...
        shifts: {
            morning: Boolean(day.shifts?.morning),
            afternoon: Boolean(day.shifts?.afternoon),
            evening: Boolean(day.shifts?.evening)
        }
    }));
