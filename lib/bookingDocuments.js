...
export function getBookingDocumentContext(booking = {}, options = {}) {
    ...
    // Issue date is when this document/record was created — stable across
    // repeat downloads, unlike "today". Estimates/Invoices then each get
    // their own second date (valid-until / due) computed from it unless the
    // booking carries an explicit override.
    const issueDateSource = booking.createdAt || booking.date;
    const issueDate = formatDate(booking.createdAt ? booking.createdAt.slice(0, 10) : booking.date);
    const validUntil = formatDatePlusDays(issueDateSource, 30);
    const paymentDue = booking.paymentDueDate
        ? formatDate(booking.paymentDueDate)
        : formatDatePlusDays(issueDateSource, 14);
...
export function buildBookingDocumentHtml(booking = {}, options = {}) {
    ...
    const totalLabel = documentLabel === "Receipt" ? "Amount paid" : "Amount due";
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .terms { margin-top: 14px; font-size: 10.5px; color: #8a94a3; line-height: 1.6; }
