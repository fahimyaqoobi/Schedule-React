...
export function buildDocumentPricingBreakdown(booking = {}, company = {}) {
    ...
    const manualDiscount = Math.min(subtotal, fixedDiscount + percentDiscountValue);
    ...
    // Tax is always calculated on the after-discount subtotal
    ...
    // Estimates and bookings show "+ HST (13%)" wording only; invoices/receipts show the calculated amount
  ...
  <div class="page">
    ...
    <div class="main-bottom">
      ...
      <div class="totals">
        ...
        ${context.pricing.fixedDiscount > 0 ? `<div class="totals-row"><span>Discount (fixed)</span><strong>-${formatMoney(context.pricing.fixedDiscount)}</strong></div>` : ""}
        ${context.pricing.percentDiscountValue > 0 ? `<div class="totals-row"><span>Discount (${booking.customDiscountPercent || ""}%)</span><strong>-${formatMoney(context.pricing.percentDiscountValue)}</strong></div>` : ""}
        ${context.pricing.promoCode && context.pricing.promoDiscount > 0 ? `<div class="totals-row"><span>${escapeHtml(context.pricing.promoName || "Promo")} (${escapeHtml(context.pricing.promoCode)})</span><strong>-${formatMoney(context.pricing.promoDiscount)}</strong></div>` : context.pricing.promoCode ? `<div class="totals-row"><span>Promo Code</span><strong>${escapeHtml(context.pricing.promoCode)}</strong></div>` : ""}
        ${context.pricing.totalDiscount > 0 ? `<div class="totals-row" style="border-top:1px solid #e2e8f0;margin-top:4px;padding-top:8px;font-weight:600;"><span>Subtotal after discounts</span><strong>${formatMoney(context.pricing.subtotalAfterDiscounts)}</strong></div>` : ""}
        ${context.pricing.showTaxAmount ? `<div class="totals-row"><span>${escapeHtml(context.pricing.taxLabel)} (${context.pricing.taxRatePercent}%)</span><strong>${formatMoney(context.pricing.taxAmount)}</strong></div>` : ""}
        ...
        <div class="total-box">
          <span>TOTAL</span>
          <span>${formatMoney(context.pricing.showTaxAmount ? context.pricing.total : context.pricing.subtotalAfterDiscounts)}</span>
        </div>
        ${!context.pricing.showTaxAmount ? `<div style="font-size:11px;color:#64748b;font-style:italic;margin-top:8px;padding:0 2px;">* ${escapeHtml(context.pricing.taxLabel)} (${context.pricing.taxRatePercent}%) is not included. It will be added at the time of invoicing.</div>` : ""}
      </div>
    </div>
    ...
    <div class="footer">
      ...
      <div class="hst">
        ${documentLabel === "Invoice" || documentLabel === "Receipt" ? `
