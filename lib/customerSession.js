...
export function getSessionPhoneCookie(cookieValue) {
    if (!cookieValue) throw new Error("Unauthorized");
    return verifyCustomerSession(cookieValue).phone;
}

// Checks cookie first, then falls back to Authorization Bearer — backward compatible.
export function getSessionPhoneAny(request) {
    const cookieVal = request.cookies?.get?.("cst")?.value;
    if (cookieVal) return getSessionPhoneCookie(cookieVal);
    return getSessionPhone(request);
}
